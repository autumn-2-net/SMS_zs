...
def loadckpt(path,configs):
    models_ssvc = ssvc(config=configs)
    # models_ssvc.load_from_checkpoint(path)
    models_ssvc.load_state_dict(torch.load(path)['state_dict'])
...
if __name__ == '__main__':
    ...
    # models_ssvc=ssvc(config=config)
    models_ssvc=loadckpt(r'D:\propj\sum_a\ckpt\aug_2encoder_1_ff0_ne\model_ckpt_steps_359999.ckpt',configs=config)
    ccx=loadds('穿越.ds','me.wav')
    melss=[]
    sss=[]
    f0e=[]
    for i in ccx:
        mel =models_ssvc.run_modelI(i,infer=True)
        melss.append(mel)
        f0e.append(i['f0'])
    for i ,f0 in zip(melss,f0e):
        sss.append(models_ssvc.vocoder.spec2wav(i.squeeze( dim=0), f0=f0))
    # for i in sss:
    wavss=torch.cat(sss,dim=0)
    torchaudio.save('cpxmn.wav', wavss.cpu().unsqueeze(0), sample_rate=44100)
